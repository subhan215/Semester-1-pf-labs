...
void numValid();
int editProfile(FILE *filePtr);
int *creditCardValid();
int main()
{
 char choice;
        printf("what you want to do login or sign up:\npress l for login and s for sign up:\n");
...
int profile(char filePath[])
{
	...
	if (counter == 4 && role == 'r')
	{
		...
		char string[5];
	printf("Enter word \"end\" for end the program.\nEnter \"OUT\" for logout \nEnter \"HOME\" for home page \nOr press any key to continue.");
	gets(string);
	if(strcmp(string , "out")==0 || strcmp(string , "OUT")==0)
	{
		main();
	}
	else if(strcmp(string , "home")==0 || strcmp(string , "HOME")==0)
	{
		char filePath[50];
		profile(filePath);
	}
	...
	else if (counter > 3 && role == 'c')
	{
		...
		if (choice == 1)
		{
			...
			printf("\nEnter hotel number which you want to book(eg. 1/2/3):");
			fflush(stdin);
			gets(hotelNo);
			...
			else
			{
				...
				if (strcmp(paymentMethodChoice, "yes") == 0)
				{
					...
					if (strcmp(resOrder.paymentMethod, "card") == 0)
					{
					    ...
					    else if(strcmp(resOrder.paymentMethod, "cash") == 0)
					    {
...
int editProfile(FILE *filePtr){
    ...
    // Edit Profile //
    if(editProfChoice == 'n' || editProfChoice == 'N'){
    	return 0;
	}
    else if(editProfChoice == 'y' || editProfChoice == 'Y') {
    	system("cls") ; 
    	f :
    	printf("\nWhat do you want to change: \n1:Email\n2:Password\n3:UserName\n") ; 
    	int changeChoice , i , j ;
		char newEmail[50] , line[50] , fileData[100][50] ; 
...
int signUpFunc() {
	...
	printf("\ncreate your password (Password should be length of 8-16)\n(It contain atleast one digit ,one special character ,one upper case ,and one lower case)\n");
    while(1){
...
int logInFunc() {
    ...
    if(fPtr == NULL) {
    	printf("Can't find your account!") ;
		goto c; 
	} else {
